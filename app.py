...
with open("agronomic_data.json", "r") as file:
    agronomic_data = json.load(file)
...
if uploaded_file:
    ...
    if pred_class in agronomic_data:
        info = agronomic_data[pred_class]
        st.markdown("### 🌱 Agronomic Details")
        st.markdown(f"- **Cultivation Time:** {info['cultivation_time']}")
        st.markdown(f"- **Ideal Humidity:** {info['humidity']}")
        st.markdown(f"- **Soil Type:** {info['soil']}")
        st.markdown(f"- **Expected Yield:** {info['yield']}")
    else:
        st.warning("No agronomic info available.")
